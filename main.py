...
def get_app_token(login_token):
    """
    根据 login_token 获取 app_token
    :param login_token: 登录 token
    :return: app_token
    """
    url = (
        f"https://account-cn.huami.com/v1/client/app_tokens?app_name=com.xiaomi.hm.health"
        f"&dn=api-user.huami.com%2Capi-mifit.huami.com%2Capp-analytics.huami.com&login_token={login_token}"
    )
    headers = {
        'User-Agent': (
            'Mozilla/5.0 (Linux; Android 10; Mi 10 Pro) AppleWebKit/537.36 '
...
def login(user, passwd):
    """
    登录并返回 login_token 和 user_id
    :param user: 用户名（手机号或邮箱）
    :param passwd: 密码
    ...
    try:
        ...
        print(f"账号 {user} 获取 code 成功")
    except Exception as err:
        print(f"获取 code 失败: {response.text}, 错误: {err}")
        return None, None

    # 使用 code 获取 token 和用户信息
    ...
    try:
        ...
        user_id = res["token_info"]["user_id"]
...
# 修改步数
def change_steps(user, user_id, app_token, step=None):
    """
    修改步数并同步到平台
    :param user: 用户名
    :param user_id: 用户 ID
    ...
    try:
        ...
        print(f"账号 {user}: 修改步数为 {step}, 返回信息: {response['message']}")
        return True
    except Exception as err:
        print(f"修改步数失败: {err}")
        return None
...
if __name__ == "__main__":
    ...
    # 遍历账号并处理
    for acc in account:
        step = acc[2] if acc[2] is not None else random.randint(RandomMin, RandomMax)

        login_token, user_id = login(acc[0], acc[1])
        if not login_token:
            print("登录失败，跳过此账号")
            continue
        ...
        if not app_token:
            print("获取 app_token 失败，跳过此账号")
            continue

        result = change_steps(acc[0], user_id, app_token, str(step))
        if not result:
            print("修改步数失败")
